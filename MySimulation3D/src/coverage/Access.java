...
//        Time[] timeNodeArray = this.getTimeNode(ai.startTime, ai.endTime, taskFinalTime.toBJTime());
        ...
        int[][] formerChinaGrid=new int[800][1400];//本函数按照旬进行步进，为得到当前旬的覆盖进度，需知道上一个旬的覆盖情况。该矩阵用于存放上个旬的覆盖情况
            ...
            while (DayStartTime.beforeOrEqual(timeNodeArray[timeNodeIndex + 1])) {
                ...
                for (int i = 0; i < 800; i++) {
                    for (int j = 0; j < 1400; j++) {
                        ...
                        if (odc.chinaGrid[i][j] != -10 && odc.chinaOneDayGrid[i][j] >= 1) {//如果[i][j]位于地面任务区域内部，且覆盖次数超过1次
                            xyList.add(new XYindex(i, j));
//                            odc.chinaGrid[i][j] += 1;//一天内覆盖多次按照一次计算
                            odc.chinaOneDayGrid[i][j] = 0;//覆盖次数重新置零，用于下一次计算
                        }
                    }
                }
            ...
            for (int i = 0; i < 800; i++) {
                for (int j = 0; j < 1400; j++) {
                    if (odc.chinaGrid[i][j] != -10) {//首先判断是否位于地面任务区域内部
                        int coverageTimes = odc.chinaGrid[i][j];//点[i][j]的当前旬实际覆盖次数
                        coverageTimes = coverageTimes >= 19 ? 19 : coverageTimes;//超过19次，则忽略
                        ao.coverageTimesArray[timeNodeIndex][coverageTimes] += 1;//覆盖次数计入AccessOutput中，
                        int xunTimes = odc.chinaGrid[i][j] - formerChinaGrid[i][j];//计算当前旬的实际覆盖次数=当前旬的总覆盖次数-上一旬的总覆盖次数
