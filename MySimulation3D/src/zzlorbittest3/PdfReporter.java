...
//            ao.progressArray[2]=0.9f;
            ...
            str1 = "图中绿色部分的区域是易于获取的区域，黄色部分的区域是较易获取的区域，蓝色部分的区域是较难获取的区域，红色部分的区域是难于获取的区域，";
            addContent(doc, str1);
            ...
            for (int i = 0; i < 5; i += 1) {
                ...
                para = new Paragraph("100%", tableFont);
                para.setAlignment(Element.ALIGN_CENTER);
                cell = new PdfPCell(para);
                cell.setHorizontalAlignment(1);
                table.addCell(cell);

                para = new Paragraph("80%", tableFont);
                para.setAlignment(Element.ALIGN_CENTER);
                cell = new PdfPCell(para);
                cell.setHorizontalAlignment(1);
                table.addCell(cell);
